...
<!DOCTYPE html>
<head>
    <title>Completed Orders- Hotel Flora-inn</title>
...
<body class="Site grey lighten-3">
		...
		<td>Sr. No.</td>	
			...
			<tbody class="" style=" padding-right: 0px; margin-right: 0px;"><tr>
				...
				<td> <%=rs.getLong(4) %> </td>
                                ...
                                <td> <%=rs.getInt(6) %> </td>
                                <td><button class="black white-text waves-effect waves-light btn" onclick="PaidOrder('<%=rs.getString(2)%>','<%=rs.getString(3)%>','<%=rs.getLong(4)%>','<%=rs.getString(5)%>','<%=rs.getInt(6)%>')"> Paid</button></td>
               		</tr>
                        </tbody> </table>
                <table class="table ">
 ...
 <footer class="black page-footer orange" style=" bottom: 0; left: 0; width: 100%;">
  ...
  <script>
       window.history.forward();
        ...
        function PaidOrder(fName,lName, mobNo, email, tabNo) {
        var url = "ajax/paidOrder.jsp?fName="+fName+"&lName="+lName+"&mobNo="+mobNo+"&email="+email+"&tabNo="+tabNo;
