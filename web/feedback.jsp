...
<!DOCTYPE html>
<head>
    <title>Feedbacks- Hotel Flora-inn</title>
  ...
  <div id="addAccount" class="modal">
	...
	<table>
		...
		<td>				
                ...
                <input class="btn black white-text" type="submit" name="submit" value="Add Account"/>	
	</form>	
      </div>
  </div>
    ...
    <!-- fetching feedback-->
 	<table class="table highlight responsive-table grey lighten-3">
	...
	<%
		ResultSet rs = Demo.fetchFeedbacks();
	int i = 1;
		while(rs.next()){
			%>
			<tbody><tr>
				...
				<td> <button class="btn black white-text"  onclick="deleteFeedback( '<%=rs.getString(1) %>' , '<%=rs.getString(2) %>' ,  '<%=rs.getString(3) %>')">Delete</button> </td>
			</tr>			
		</tbody>
			<% } 
          ...
          <div class="col l3 s12 offset-l1">
            ...
            <ul>
                <li><a class="white-text" href="orders.jsp">Orders</a></li>           
                 ...
                 <li><a class="white-text modal-trigger" href="#addAccount" >Add Account</a></li>   
                 <li> <a href="forStaff.jsp" class="white-text" >Sign Out</a> </li>
            </ul>
          </div>
        </div>
      </div>
...
function deleteFeedback(name, email, feedback) {
  var url = "ajax/deleteFeedback.jsp?name="+name+"&email="+email+"&feedback="+feedback;
